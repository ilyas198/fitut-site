...
/* ---- HERO : fondu-croisé + barre de progression ----
   Le nombre de slides est lu depuis le DOM : ajouter une slide
   dans le HTML suffit, aucune valeur codée en dur ici. */
(function initHero() {
  ...
  // Génère une puce de progression par slide
  ...
  function relancer() {
    clearTimeout(timer);
    if (reduit || slides.length < 2) return;
    timer = setTimeout(() => aller(index + 1), DUREE);
  }
